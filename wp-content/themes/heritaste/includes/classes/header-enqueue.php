...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'heritaste' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
		);

		$scripts = apply_filters( 'HERITASTE/includes/classes/header_enqueue/scripts', $scripts );
		/**
		 * Enqueue the scripts
		 *
		 * @var array
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( HERITASTE_NONCE ),
		);

		wp_localize_script( 'jquery', 'heritaste_data', $header_data );

		if ( heritaste_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', heritaste_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		...
		$styles = apply_filters( 'HERITASTE/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "heritaste-{$name}", $style );
			} else {
				wp_enqueue_style( "heritaste-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = heritaste_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= heritaste_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'heritaste-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'heritaste-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$data = \HERITASTE\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'heritaste' . '_options-mods' );

		if ( $custom_font = heritaste_set( $settings, 'theme_custom_font' ) ) {

			$styles .= apply_filters('heritaste_redux_custom_fonts_load', $custom_font );
