...
class Visual_Composer {
	...
	function init() {
		...
		$maps = apply_filters( 'heritaste_vc_map', $this->maps );
		...
		foreach ( $maps as $value) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( function_exists( 'wpheritaste_shortcode' ) ) {
					$tag = esc_attr( heritaste_set( $data, 'base' ) );

					wpheritaste_shortcode( $tag, array( $this, 'output' ) );
				}
			}
		}
	}
	...
	function output( $atts, $content = null, $tag ) {
		...
		heritaste_template_load( 'shortcodes/' . $tag . '.php', compact( 'atts', 'content', 'tag' ) );
	...
	function get_file( $tag ) {
		...
		$file = heritaste_template( 'includes/resource/vc_map/'.$tag . '.php' );
		$file = apply_filters( "heritaste_vc_map_file_{$tag}", $file );
	...
	function shortcodeParams( $tag ) {
		...
		if ( file_exists( $file ) ) {
			...
			return heritaste_set( $data, 'params' );
		}
	...
	function toggle( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array( esc_html__( 'Yes', 'heritaste' ) => 'true' );
		if ( ! empty( $values ) ) {
