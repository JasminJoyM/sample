...
use HERITASTE\Includes\Classes\Header_Enqueue;
use HERITASTE\Includes\Classes\Options;
...
class Base {
	...
	public static function instance() {
		...
		if ( isset( $GLOBALS['heritaste_base'] ) ) {
			return $GLOBALS['heritaste_base'];
		}
	...
	function option( $key = '' ) {
		...
		$options = (array) get_theme_mod( 'heritaste' . '_options-mods' );

		$dn = heritaste_dot( $options );
	...
	function get_meta( $key = '', $id = '' ) {
		...
		$id = ( $id ) ? $id : heritaste_set( $post, 'ID' );
