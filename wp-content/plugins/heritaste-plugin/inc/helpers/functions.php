...
function heritaste_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('heritaste_redux_custom_fonts_load', 'heritaste_redux_custom_fonts_load');
function heritaste_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = heritaste_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/heritaste/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [heritaste_social_share_output description]
 *
 ...
 */
function heritaste_social_share_output( $icon, $color = false ) {
...
function heritaste_get_social_icons()
{
	...
	if( heritaste_set( $options, 'social_media' ) && is_array( heritaste_set( $options, 'social_media' ) ) )
	{
		$total = count( heritaste_set( $options, 'social_media' ) ) - 2;
		foreach( heritaste_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = heritaste_set( $social_icon, 'title');
			$class = heritaste_set( $social_icon, 'class');
			$link = heritaste_set( $social_icon, 'social_link');
			$icon = heritaste_set( $social_icon, 'social_icon');
...
add_action('heritaste_social_share_output_two', 'heritaste_social_share_output_two');
...
function heritaste_get_social_icon()
{
